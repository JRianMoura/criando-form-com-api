    ...
    <form className="flex flex-col" onSubmit={handleSubmit(onSubmit)}>
      <div className="mb-2 flex flex-col">
        ...
        <input type="text" id="name" {...register("name", {
          required: 'O nome precisa ser preenchido.', maxLength: {
            value: 255,
        ...
        <div className="relative flex items-center">
          ...
          <span className="absolute right-3">
            <button type="button" onClick={() => setPasswordVisible(!isPasswordVisible)}>
              {
                !isPasswordVisible ? <EyeIcon size={20} className="text-slate-600 cursor-pointer" /> : <EyeOffIcon
        ...
        <div className="relative flex items-center">
          ...
          <span className="absolute right-3">
            <button type="button" onClick={() => setPasswordVisible2(!isPasswordVisible2)}>
              {
                !isPasswordVisible2 ? <EyeIcon size={20} className="text-slate-600 cursor-pointer" /> : <EyeOffIcon
                      className="text-slate-600 cursor-pointer"
                      size={20}
                    />
          }
          ...
          <p className="text-xs text-red-400 mt-1">{errors.password_confirmation
?.message as string}</p>
      </div>
      <div className="mb-2 flex flex-col">
        <label htmlFor="phone">Telefone Celular</label>
        <input type="text" id="phone" {...registerWithMask('phone', '(99) 99999-9999', {
          required: 'O telefone precisa ser preenchido', pattern: {
            value: /^\(\d{2}\) \d{5}-\d{4}$/,
            message: "Telefone inválido"
        }})}/>
      </div>
      <p className="text-xs text-red-400 mt-1">{errors.phone
?.message as string}</p>
      <div className="mb-2 flex flex-col">
        ...
        <p className="text-xs text-red-400 mt-1">{errors.cpf
?.message as string}</p>
      </div>
      <div className="mb-2 flex flex-col">
        ...
        <p className="text-xs text-red-400 mt-1">{errors.zipcode
?.message as string}</p>
      </div>
      <div className="mb-2 flex flex-col">
      ...
      <div className="mb-4 flex flex-row gap-4">
        ...
        <div className="flex flex-col">
          ...
          <input className="disabled:bg-slate-200 max-w-10" type="text" id="uf" disabled value={address.state}/>
        </div>
        
      </div>
      {/* terms and conditions input */}
      <div className="mb-4">
        <input type="checkbox" id="terms" className="mr-2 accent-slate-500" {...register('terms', {required: "Os termos e condições devem ser aceitos"})} />
        <label
        ...
        <p className="text-xs text-red-400 mt-1">{errors.terms
?.message as string}</p>
      </div>
